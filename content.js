...
(function () {
  ...
  let globalMode = UNBAR.DEFAULT_MODE; // applies to sites without an override
  let siteMode = null; // this site's override, or null to follow the global
  let currentMode = UNBAR.DEFAULT_MODE; // the effective mode actually applied
  let fillStrength = DEFAULT_FILL; // how far Smart zooms from original to full

  const effectiveMode = () => siteMode || globalMode;
  ...
  function fillBox(video) {
    ...
    for (let i = 0; el && i < 4; i++) {
      if (el.clientWidth > (box.clientWidth || 0)) box = el;
      el = el.parentElement;
    }
    return box || video.parentElement || document.documentElement;
  }

  // Compute the transform that removes the bars for this mode, based on how the
  // video's rendered box compares to the box it should fill. Returns "" when we
  // can't measure (object-fit then does what it can on its own).
  function coverScale(video, box, extra) {
  ...
  function desiredStyle(video, mode) {
  ...
  function styleVideo(video, mode) {
    if (mode.id === "off") {
  ...
  function watch(video) {
    ...
    const mo = new MutationObserver(() => {
      const mode = UNBAR.byId(currentMode);
  ...
  function reapply() {
    ...
    raf = requestAnimationFrame(() => applyToAll(UNBAR.byId(currentMode)));
  }
  ...
  function showToast(text) {
    ...
    toastTimer = setTimeout(() => {
      toastEl && toastEl.classList.remove("unbar-toast--visible");
    }, 1200);
  }
  ...
  function setSiteMode(id) {
    chrome.storage.local.get(SITES_KEY, (res) => {
      const sites = res[SITES_KEY] || {};
      sites[SITE] = id;
  ...
  // it if it changed.
  function refresh({ toast } = {}) {
    const next = effectiveMode();
    if (next === currentMode) return;
    currentMode = next;
    applyToAll(UNBAR.byId(next));
    ...
    if (toast && document.querySelector("video")) showToast(UNBAR.byId(next).label);
  }
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    ...
    if (changes[FILL_KEY]) {
      ...
      if (currentMode === "smart") applyToAll(UNBAR.byId("smart"));
    }
  ...
  function videoForWheel(x, y) {
    const fs = document.fullscreenElement || document.webkitFullscreenElement;
    if (fs) return fs.matches("video") ? fs : fs.querySelector("video");
  ...
  function onWheel(e) {
    ...
    const next = UNBAR.step(currentMode, delta);
    if (next !== currentMode) setMode(next);
    else showToast(UNBAR.byId(currentMode).label); // clamped at the end
  }
  ...
  const observer = new MutationObserver((mutations) => {
    ...
    for (const m of mutations) {
      for (const node of m.addedNodes) {
        ...
        if (node.tagName === "VIDEO") styleVideo(node, mode);
        else if (node.querySelectorAll) {
          node.querySelectorAll("video").forEach((v) => styleVideo(v, mode));
        }
      }
    }
  });
  ...
  function onVideoLoad(e) {
    if (e.target && e.target.tagName === "VIDEO" && currentMode !== "off") {
      styleVideo(e.target, UNBAR.byId(currentMode));
    }
  }
  ...
  chrome.storage.local.get([KEY, SITES_KEY, FILL_KEY], (res) => {
    ...
    if (currentMode !== "off") applyToAll(UNBAR.byId(currentMode));
